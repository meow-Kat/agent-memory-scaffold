...
3. Execute: split into docs/tasks.md → coder → tester → verifier → all green → commit → update progress.md/tasks.md → next.
   ...
   - **UI gate**: `architecture.md` `Frontend / UI` ≠ none AND a task touches UI → after that task's tester goes green, browser-verify via a browser-testing sub-agent (launch info from the Frontend field) BEFORE the verifier runs; tools without a browser-testing sub-agent → unsupported-by-tool, and the report must state the change is unverified-in-browser.
