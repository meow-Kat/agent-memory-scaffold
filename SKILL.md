...
2. Branch:
   ...
   - Exists / maintained → AUDIT (read-only, make NO changes), grading against "Target structure":
     ...
     k. **Model tiering present & operable**: the work loop defines orchestrator self-judging task difficulty + coder/tester → opus/sonnet/haiku mapping + verifier fixed opus; Claude Code uses the dispatch `model` override (real mechanism, frontmatter `model:` fallback). Non-Claude tools → record as unsupported-by-tool, NOT Missing (same handling as item h).
